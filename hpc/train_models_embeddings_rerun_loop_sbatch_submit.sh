...
for SEED_SIZE in 50; do
	for TRANSFORMER_NUM in 10 11; do
		...
		for (( i = 0; i < 100; i++ )); do
			sbatch -p ckpt -A stf-ckpt --mail-user $EMAIL --chdir $AUTOREVIEW_PROJECT_DIR --output $DATA_SUBDIR/seed_papers_size_${SEED_SIZE}/select_wos_id_and_train_models_rerun20200130_${i}_features${TRANSFORMER_NUM}.out hpc/batch_scripts/select_wos_id_and_train_models_embeddings_rerun.sbatch $ID_LIST $i $DATA_SUBDIR/seed_papers_size_${SEED_SIZE}/ $TRANSFORMER_NUM /gscratch/stf/jporteno/wos_201912/hpc/data/title_embeddings/
		done
		echo ""
	done
done
